...
# built a classifier from *known* labels of the initial 
# data to determine whether observed data belonged to 
# elliptic or hyperbolic parameter types, and then pieced 
# together a global inverse solution from local inverse solutions?
# This is the most ideal scenario to determine the best possible 
# case for our inverse solution.
...
# Now add noise to observations, use less initial and observed samples, 
# use LUQ to filter based on known labels of reduced initial sample set, 
# classify, and learn QoI.
# This will produce the operational case studies for limited datasets 
# that better reflect real-world conditions that are then compared to the ideal case above.
...
for j in range(num_sets):
    ...
    print(f'Weighted diagnostic for set {num_sets+1}: {weighted_diagnostic}')
